...
// Model Under Test
#include "top_coupled_Vending_Machine.hpp"
...
#include "Vending_Machine_Variant_Goblin.hpp"
	...
	int main() {
		std::cout << "------------------ running tests for: Vending_Machine ------------------" << std::endl;

		std::filesystem::path cpp_file_path = __FILE__;

		std::string model_under_test_name = "Vending_Machine";
		...
		for(int test_set_enumeration = 1; test_set_enumeration <= test_set_size; test_set_enumeration++)
		{
			...
			auto td = std::make_shared<top_Vending_Machine>("Top_"+model_under_test_name, test_set_enumeration, log_file, t, c, p, co);
