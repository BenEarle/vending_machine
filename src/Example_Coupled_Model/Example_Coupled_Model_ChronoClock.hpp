...
#ifndef Example_Coupled_Model_RT_CLOCK_CHRONO_HPP
#define Example_Coupled_Model_RT_CLOCK_CHRONO_HPP
...
#include "Example_Coupled_Model_async_inputs.hpp"
...
class ChronoClock : RealTimeClock {
    ...
    std::shared_ptr<Example_Coupled_Model_Async_Inputs> async_handle;
    bool IE;
    double startTime;
    public:
    ...
    [[maybe_unused]] explicit ChronoClock(std::shared_ptr<Coupled> model) : ChronoClock() {
        ...
        this->async_handle = std::make_shared<Example_Coupled_Model_Async_Inputs>();
    }
    ...
    * */
    double waitUntil(double timeNext) override {
        ...
        // port for propagating to CoupledInput1
        cadmium::Port<int> CoupledInput1;
        CoupledInput1 = pseudo.addOutPort<int>("CoupledInput1");
        while(T::now() < rTimeLast) {
            if(IE){
                if (async_handle->ISR_flag_CoupledInput1()) {
                    auto data = async_handle->ISR_decode_CoupledInput1();
                    auto epoch = T::now().time_since_epoch();
                    double time_now = std::chrono::duration<double>(epoch).count();
                    CoupledInput1->addMessage(data);
                    top_model->getInPort("CoupledInput1")->propagate(CoupledInput1);
                    rTimeLast = T::now();
                    break;
                } 
...
#endif // Example_Coupled_Model_RT_CLOCK_CHRONO_HPP   
