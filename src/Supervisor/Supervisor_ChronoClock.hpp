...
#ifndef Supervisor_RT_CLOCK_CHRONO_HPP
#define Supervisor_RT_CLOCK_CHRONO_HPP
...
#include "Supervisor_async_inputs.hpp"
...
class ChronoClock : RealTimeClock {
    ...
    std::shared_ptr<Supervisor_Async_Inputs> async_handle;
    bool IE;
    double startTime;
    public:
    ...
    [[maybe_unused]] explicit ChronoClock(std::shared_ptr<Coupled> model) : ChronoClock() {
        ...
        this->async_handle = std::make_shared<Supervisor_Async_Inputs>();
    }
    ...
    * */
    double waitUntil(double timeNext) override {
        ...
        // port for propagating to abort
        cadmium::Port<int> abort;
        abort = pseudo.addOutPort<int>("abort");
        while(T::now() < rTimeLast) {
            if(IE){
                if (async_handle->ISR_flag_abort()) {
                    auto data = async_handle->ISR_decode_abort();
                    auto epoch = T::now().time_since_epoch();
                    double time_now = std::chrono::duration<double>(epoch).count();
                    abort->addMessage(data);
                    top_model->getInPort("abort")->propagate(abort);
                    rTimeLast = T::now();
                    break;
                } 
...
#endif // Supervisor_RT_CLOCK_CHRONO_HPP   
